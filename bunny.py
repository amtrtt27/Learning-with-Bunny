...
def random_abcd(multiple_number):
    ...
    for i in range(4):
        ...
        current = i
        if i == correct_letter:
            list_button[i].config(command = lambda: change_color_button(list_button[current], 'green'))
        else:
            list_button[i].config(command = lambda: change_color_button(list_button[current], 'red'))
...
def flashcard_frame_definition(word, flashcard_number):
    ...
    if back_flashcard_number <= 0:
        back_flashcard_number = 0
    back_button(lambda: flashcard_frame_word(word_have[back_flashcard_number], back_flashcard_number))
